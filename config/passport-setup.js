...
passport.use(
  new GoogleStrategy({
    ...
    const body = {
      ...
      const response = await db.query('SELECT * FROM "public"."users" WHERE "oauth_data" = $1', [body.googleid])
     //if user does not exist, query the db again to add the user
      if (!response.rows[0]) await db.query(`INSERT INTO users (name, oauth_data) VALUES ($1, $2) RETURNING *`, [body.usernam, body.googleid])
       //else query the database to find the ID of the newly added user 
       const newUser = await db.query('SELECT * FROM "public"."users" WHERE "oauth_data" = $1', [body.googleid])
       done(null, newUser);
  } catch (error) {
